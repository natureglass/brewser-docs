...
then uploads to your Brewser account a moment later. Nothing waits on the
network, and saving works offline; the upload happens once you're back online.
...
- **Signed out**: saves still work, instantly, but stay only on that device.
  They sync once you [sign in](/docs/features/accounts).
...
localStorage, which always wins. The user may have saved offline, or a device
clock may be wrong, so the local copy is authoritative for that device. To
bring in a copy made elsewhere, you explicitly `pull()`.
...
| `brewser.sync()` | `Promise` | Force an immediate push (skips the debounce); useful for a "Sync now" button. |
...
Records and whole-save are the same storage; records interpret the blob as an
array. Don't mix a non-array `save()` blob with record helpers on the same app;
if the blob isn't an array, the record helpers start from an empty list rather
than throwing.
...
| --- | --- | --- | --- |
| `.config({ order })` | the leaderboards object | none | `'desc'` high-wins (default) or `'asc'` low-wins. Call once. |
| `.order()` | `'desc' \| 'asc'` | none | The direction currently declared on the client. |
...
brewser.leaderboards.submit(1200, { name: 'Alice' }).then(res => {
  if (!res.ok) { /* res.reason === 'unauth' means prompt sign-in */ return; }
...
`list()` is safe signed-out (it can't flag which row is "you"); everything
...
token arrives automatically, so your app doesn't manage auth. The SDK finds it
in this order:
...
| `token` | none | Hand in an auth token explicitly. |
...
A minimal but complete integration that loads on boot, saves on change, submits
a score, and shows the top 10:
