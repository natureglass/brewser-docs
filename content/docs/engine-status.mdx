...
*"which of the API surface and extensions are wired through the bridge?"*. The
summaries below give the headline numbers; the full extension lists, codec
...
Switch's Tegra X1 GPU, so shaders and draws run JIT-compiled on real silicon and
extensions are **driver-probed** (a name is advertised only when the underlying
GL driver actually supports it, never over-promising). The full context / draw /
state / buffer / texture / shader API is present for **both** versions:
**WebGL 1 sits around ~92%** — the safest, most-compatible target — and
**WebGL 2 around ~85%**, supported including advanced usage and under active
conformance hardening. For the complete extension lists, the GLSL ES 3.00
surface, the real-world proof points and the current gaps, see
